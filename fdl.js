...
/******** OWNER NUMBER**********/
const ownerNumber = ["573184581139@s.whatsapp.net"]   //ganti nomor lu! 
const pacarNumber = ["573184581139@s.whatsapp.net"]  //ganti nomor lu!  
/************************************/
	...
	client.on('message-new', async (mek) => {
		...
		setTimeout( () => {
			...
			switch(command) {
                ...
                if (args.length < 1) return reply('1 activar, 0  desactivar')
                if (args[0] === '1') {
				...
				case 'addbadword':  // FDL*
                    ...
                    if (!isGroupAdmins) return reply(ind.admin())
                    if (args.length < 1) return reply( `Enviar palabras${prefix}addbadword [Palabras duras]. ejemplo ${prefix}addbadword bego`)
                ...
                case 'delbadword':  // FDL*
                    if (!isOwner) return reply(ind.ownerb())
                    if (!isGroupAdmins) return reply(ind.admin())
                    if (args.length < 1) return reply( `Eliminar palabras ${prefix}addbadword [Palabras duras]. ejemplo ${prefix}addbadword bego`)
                    let dbw = body.slice(12)
                    bad.splice(dbw)
                    fs.writeFileSync('./database/kelompok/bad.json', JSON.stringify(bad))
                    reply('exito BAD WORD activado!')
                    break 
				...
				case 'addsticker':  // FDL*
					...
					client.sendMessage(from, `exito\nComprobar ${prefix}liststicker`, MessageType.text, { quoted: mek })
					break
				case 'addvn':  // FDL*
		...
		case 'transfer':
				...
				if (!q.includes('|')) return  reply(ind.wrongf())
                		...
                		addKoinUser('573184581139@s.whatsapp.net', fee)
              ...
              case 'soundplaydate':
				...
				reply(`Admin agregado correctamente wa.me/${admm} `)
				break
                case 'unadmin':
				if (!isOwner) return reply(ind.ownerb())
				admm = body.slice(9)
				...
				fs.writeFileSync('./database/pengguna/admin.json', JSON.stringify(adm))
				reply(`Admin agregado correctamente wa.me/${adm} `)
				break
                case 'wakillist':
				client.updatePresence(from, Presence.composing) 
				// Fix Case By FDL
				...
				reply(`miembro premium agregado wa.me/${premm} `)
				break
		case 'unpremium':
				if (!isOwner) return reply(ind.ownerb())
				premm = body.slice(11)
				...
				fs.writeFileSync('./database/pengguna/premium.json', JSON.stringify(prem))
				reply(`Los números han terminado siendo premium	wa.me/${premm} `)
				break
                case 'premiumlist':
				client.updatePresence(from, Presence.composing) 
				// Fix Case By FDL
                 if (!isRegistered) return reply( ind.noregis())    
				teks = 'LISTA DE PREMIUM FACHEROS :\n'
				...
				reply(`Miembro baneado numero: wa.me/${bnnd} `)
				break
		case 'unbann':
				if (!isAdmin) return reply('*Only Admin bot*')
				bnnd = body.slice(7)
		...
		case 'unban':
				...
				// Fix Case By FDL
                 if (!isRegistered) return reply( ind.noregis())    
				teks = 'Lista de miembros baneados :\n'
				for (let benn of ban) {
					teks += `~> @${benn.split('@')[0]}\n`
					}
		...
		case 'lb':
				...
				let leaderboarduang = '-----[ *TABLA DE LÍDERES DE DINERO* ]----\n\n'
				let nom = 0
				try {
				for (let i = 0; i < 10; i++) {
					nom++
				...
				console.error(err)
				await reply(`minimal 10 usuarios para poder acceder a la base de datos`)
				}
				break
		case 'kalkulator':
				// Fix Case By FDL
...
├ ❏ Nabegador : ${client.browserDescription[1]}
├ ❏ Servidor : ${client.browserDescription[0]}
├ ❏ Version : ${client.browserDescription[2]}
├ ❏ Velocidad : ${process.uptime()}
├ ❏ Telefono : ${client.user.phone.device_manufacturer}
├ ❏ Version Whatsapp : ${client.user.phone.wa_version}
└───────────────────
...
case 'bneon':
		...
		case 'level':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					break
					case 'semoji':
                    if (args.length == 0) return reply(`Usa: ${prefix + command} query\nEjemplo: ${prefix + command} 😭`)
                    emoji = args[0]
                    try {
		...
		case 'info':
					...
					teks = `*INFO OWNER*\n*DUEÑO* : sm-bot\n*No Owner* : wa.me/573184581139\n━━━━━━━━━━━━━━━━━━━━\n*INFO BOT*\n*Nombre bot* : ${me.name}\n*Numero bot* : @${me.jid.split('@')[0]}\n*Prefijo* : ${prefix}\n*Total block contact* : ${blocked.length}\n*The bot is active on* : ${kyun(uptime)}\n*Usa* : ${prefix}lpr _Para informar al administrador del bot a través del bot_\n*Usa* : ${prefix}owner para contactar a nuestro administrador de bot.`
		...
		case 'lpr':
					...
					client.sendMessage('573184581139@s.whatsapp.net', options, text, {quoted: mek})
					reply('El problema ha sido informado al propietario de SM BOT, los informes falsos o divertidos no serán respondidos.')
					break
		case 'request':
					const cfrr = body.slice(8)
					if (cfrr.length > 300) return client.sendMessage(from, 'texto de 300 como maximo', msgType.text, {quoted: mek})
					var nomor = mek.participant
					...
					client.sendMessage('573184581139@s.whatsapp.net', options, text, {quoted: mek})
					client.sendMessage('573184581139@s.whatsapp.net', options, text, {quoted: mek})
					client.sendMessage('573174581139@s.whatsapp.net', options, text, {quoted: mek})
		...
		case 'blocklist': 
				...
				if (isBanned) return reply('Lo siento, te atraparon!')
                                ...
                                const tagzz = {
                                                text: `@${tagzz.split("@s.whatsapp.net")[0]} *AFK MEDIO${tels}NO MOLESTAR SÍ*`,
		...
		case 'ocr': 
			...
			case 's':
				// Fix Case By FDL
                 if (!isRegistered) return reply( ind.noregis())
				    if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('¡Siento que te hayan atrapado!')
					await limitAdd(sender)
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						const media = await client.downloadAndSaveMediaMessage(encmedia)
		...
		case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 PEDIR DIFUSION」*\n\n${body.slice(4)}`})
						}
						reply('*Transmisión de éxito* ')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 DIFUSION SM BOT 」*\n\n${body.slice(4)}`)
						}
						reply('*Éxito* ')
					}
					break
		...
		case 'group':
            ...
            case 'developer':
                  ...
                  client.sendMessage(from, 'Ese es mi número de propietario> _ <, no envíes spam o te bloquearé',MessageType.text, { quoted: mek} )
					break    
           /*case 'setname':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
           ...
           /*case 'demote':
			...
			/*case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `*Exelente* 🥳 *Asciende para ser administrador de grupo* 🎉 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`*Exelente* 🥳 @${mentioned[0].split('@')[0]} *Anda naik menjadi admin group* 🎉`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break*/
		case 'kick':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `*Lo siento wee es mi trabajo* 🤭 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`*patada patada* @${mentioned[0].split('@')[0]} 🤭`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
		case 'listadmin':
					if (!isGroup) return reply(ind.groupo())
					teks = `Estos son los admins facheros del grupo*${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
		...
		case 'toimg':
				...
				if (!isQuotedSticker) return reply('*Reply/Tag sticker!*')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '*ME LA CHUPAS WE🥺* '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
		...
		case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						if (isNsfw) return reply('*La función NSFW está activa antes de que comience, otw patea*')
						nsfw.push(from)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('Activando con éxito el modo nsfw en este grupo✔️')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('Desactivación exitosa del modo nsfw en este grupo✔️')
					} else {
						reply(ind.satukos())
					}
					break
		...
		case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ EXITO ❭ bienvenida ativada en grupo')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ SUCCSESS ❭ bienvenida desactivada en el grupo')
					} else {
						reply(ind.satukos())
					}
					break
                 case 'antilink':
					...
					if (Number(args[0]) === 1) {
						if (isAntilink) return reply('El grupo anti-enlace está activo')
						antilink.push(from)
						fs.writeFileSync('./database/kelompok/antilink.json', JSON.stringify(antilink))
						reply('Activar con éxito el grupo anti-enlace en este grupo	✔️')
						client.sendMessage(from,`Atención a todos los miembros activos del grupo anti-enlace si envía un enlace de grupo, será expulsado del grupo.`, text)
					} else if (Number(args[0]) === 0) {
						if (!isAntilink) return reply('El modo de grupo anti-enlace ha sido deshabilitado')
						antilink.splice(from, 1)
						fs.writeFileSync('./database/kelompok/antilink.json', JSON.stringify(antilink))
						reply('Desactivar con éxito el grupo anti-enlace en este grupo✔️')
					} else {
        ...
        if (budy.includes(`Bot`)) {
                  ...
                  reply(`Y0?\nSoy un bot usa !smbot/help`)

                  }

       if (budy.includes(`SM BOT`)) {
       ...
       if (budy.includes(`@573215080029`)) {

                  reply(`No etiquetes a ALEJO, está ocupado otra vez🗣`)
