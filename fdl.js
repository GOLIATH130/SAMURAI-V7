...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:SMBOT\n' //ganti nama lu! 
            + 'ORG: SM;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=573184581139:+57 318-458-1139\n'  //ganti nomor lu! 
            + 'END:VCARD'
prefix = '!'
blocked = []   
limitawal = 10000
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Joder que bueno verte por aca🥺\n\nUser : @${num.split('@')[0]}\nBienvenido a Group : *${mdata.subject}*\n\nUsa *${prefix}smbot*Para ver los comandos`
				...
				teks = `*Hasta que te fuiste 🌚* @${num.split('@')[0]}igual no te queríamos bye👋🥺`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
        ...
        if (levelRole <= 2) {
           ...
           var prema = 'Free'
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    return reply("SIGUE DICIENDO DONG!! 😠")
                        .then(() => client.groupRemove(from, sender))
                        .then(() => {
                            client.sendMessage(from, `*「 ANTI PALABRA 」*\nEl bot te sacará por ser grosero!`, text ,{quoted: mek})
                        }).catch(() => client.sendMessage(from, `Porque no soy adm o te sacaria fuera -_-`, text , {quoted : mek}))
                } else {
                    return reply( "Por favor, no digas malas palabras 😇")
                }
            }
        }
		...
		if (isGroupAdmins) return reply('Admin es gratuito :)')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*「 ANTILINK 」*\n\n${sender.split("@")[0]} Si detecto un enlace \nTe pateare el tracero!`)
		setTimeout( () => {
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("2 segundos wee")
		}, 1000)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("3 segundos wee")
		}, 0)
	}
			...
			switch(command) {
                ...
                if (args.length < 1) return reply('1 para encender, 0 para apagar')
                if (args[0] === '1') {
		...
		case 'transfer':
              ...
              case 'soundplaydate':
				...
				client.sendMessage(from, Smbot, MessageType.audio, { quoted: mek, ptt: true })
				await limitAdd(sender)
				break
                case 'admin':
				if (!isOwner) return reply(ind.ownerb())
				...
				reply(`Bot de administración agregado correctamentewa.me/${admm} `)
				break
                case 'unadmin':
				if (!isOwner) return reply(ind.ownerb())
				admm = body.slice(9)
				...
				fs.writeFileSync('./database/pengguna/admin.json', JSON.stringify(adm))
				reply(`Bot de administración agregado correctamente wa.me/${adm} `)
				break
                case 'wakillist':
				client.updatePresence(from, Presence.composing) 
				// Fix Case By FDL
                 if (!isRegistered) return reply( ind.noregis())    
				teks = 'Esta es la lista de admins facheros 😎 :\n'
				for (let admm of adm) {
					teks += `~> @${admm.split('@')[0]}\n`
					}
				...
				reply(`Usuario premium  wa.me/${premm} `)
				break
		case 'unpremium':
				if (!isOwner) return reply(ind.ownerb())
				premm = body.slice(11)
				...
				fs.writeFileSync('./database/pengguna/premium.json', JSON.stringify(prem))
				reply(`Usuario premium agregado wa.me/${premm} `)
				break
                case 'premiumlist':
				client.updatePresence(from, Presence.composing) 
				// Fix Case By FDL
                 if (!isRegistered) return reply( ind.noregis())    
				teks = 'Lista de miembros premium :\n'
				...
				reply(`Elimino el número con éxito : wa.me/${bnnd} `)
				break
		case 'unbann':
				if (!isAdmin) return reply('*Only Admin bot*')
				bnnd = body.slice(7)
				...
				ban.push(`${bnnd}@s.whatsapp.net`)
				fs.writeFileSync('./database/pengguna/banned.json', JSON.stringify(ban))
				reply(`Se ha baneado un miembro : wa.me/${bnnd} `)
				break
		case 'unban':
				if (!isOwner) return reply(ind.ownerb())
				bnnd = body.slice(7)
		...
		case 'lb':
				...
				let leaderboardlvl = '-----[ *NIVEL DE CLASIFICACIÓN* ]----\n\n'
				let leaderboarduang = '-----[ *CLASIFICACIÓN DE DINERO* ]----\n\n'
				let nom = 0
				try {
				for (let i = 0; i < 10; i++) {
					nom++
		...
		case 'giftlimit': 
				...
				const nomerr = args[0].replace('@','')
                		...
                		var found = false
							...
							*「 GIFT KUOTA LIMIT 」*
							• User : @${d.id.replace('@s.whatsapp.net','')}
	...
	break
		...
		case 'daftar':
                			...
                			if (umurUser < 12) return reply(`your age is too young minimum 12 years`)
                    			...
                    			checkLimit(sender)
                    			console.log(color('[REGISTRO]'), color(time, 'yellow'), 'Nmbre:', color(namaUser, 'cyan'), 'Edad:', color(umurUser, 'cyan'), 'Serial:', color(serialUser, 'cyan'), 'in', color(sender || groupName))
                			} else {
                    			addRegisteredUser(sender, namaUser, umurUser, time, serialUser)
                    			await client.sendMessage(from, ppimg, image, {quoted: mek, caption: ind.registered(namaUser, umurUser, serialUser, time, sender)})
                    			addATM(sender)
                    			addLevelingId(sender)
                    			checkLimit(sender)
                    			console.log(color('[REGISTRO]'), color(time, 'yellow'), 'Nombre:', color(namaUser, 'cyan'), 'Edas:', color(umurUser, 'cyan'), 'Serial:', color(serialUser, 'cyan'))
                			}
				        break
		case 'mining':
					// Fix Case By FDL
...
├ ❏ Nabegadro : ${client.browserDescription[1]}
...
break									
		case 'crosslogo':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + commanfdl SMBOT`)
                    txt = args.join(" ")
                    reply('[❕] Loading')
...
case 'bneon':
		...
		case 'leave':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'Adios we🗣', text) // ur cods
					}, 0)
					break
		case 'bc': 
					if (!isOwner) return reply(ind.ownerb()) 
		...
		case 'add':
					...
					if (args[0].startsWith('08')) return reply('Usar el código de país')
					try {
		...
		case 'kick':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `*Adios wee lo siento pero es mi trabajo* 🤭 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
		...
		case 'toimg':
				...
				if (!isQuotedSticker) return reply('*Reply/Tag sticker!*')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '*SM BOT * '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
		...
		case 'clone':
					...
					if (args.length < 1) return reply(' *ETIQUETAS PARA CLONAR!!!* ')
